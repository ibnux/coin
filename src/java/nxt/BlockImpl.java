    ...
    void apply() {
        ...
        if (this.height > 3) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f %s to forger at height %d", ((double)backFees[i])/Constants.FEE_NXT, Constants.COIN_SYMBOL, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f %s at height %d", ((double)totalBackFees)/Constants.FEE_NXT, Constants.COIN_SYMBOL, this.height);
        }
