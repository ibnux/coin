...
jar/exe/dmg package, or run "java -jar NUX-client.jar" to start the
...
desktop icons to start the NUX server.


Using the NUX-client.zip package:

Unpack the NUX-client.zip package and open a shell in the resulting NUX
...
where the NUX UI should now be available.
...
to the NUX installation directory, as the use of special characters may result
...
values. You do not need to delete the defaults from NUX-default.properties, the
settings in NUX.properties override those in NUX-default.properties. This way,
when upgrading the software, you can safely overwrite NUX-default.properties
with the updated file from the new package, while your customizations remain
safe in the NUX.properties file.


How to contribute?

There are many ways to contribute to NUX. Here are some examples:
...
The NUX software is a client-server application. It consists of a java server
...
requests which other NUX client applications could use.
...
The default NUX client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file NUX.log, which gets
overwritten at restart. In case of an error, the NUX.log file may contain
...
nxt.jar file using the enclosed jar.sh script, and then NUX.jar should be
